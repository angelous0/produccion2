...
export const Cuellos = () => {
  ...
  const { sensors, handleDragEnd, isSaving, modifiers } = useSortableTable(items, setItems, 'cuellos');

  const fetchItems = async () => {
    try {
      const res = await axios.get(`${API}/cuellos`);
      setItems(res.data);
    } catch {
      toast.error('Error al cargar cuellos');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = guard(async (e) => {
    ...
    try {
      if (editingItem) {
        await axios.put(`${API}/cuellos/${editingItem.id}`, formData);
        toast.success('Cuello actualizado');
      } else {
        await axios.post(`${API}/cuellos`, formData);
        toast.success('Cuello creado');
      }
      setDialogOpen(false);
      setEditingItem(null);
      ...
      toast.error(err.response?.data?.detail || 'Error al guardar cuello');
    }
  });
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('¿Estás seguro de eliminar este cuello?')) return;
    try {
      await axios.delete(`${API}/cuellos/${id}`);
      toast.success('Cuello eliminado');
      fetchItems();
    } catch (err) {
      toast.error(err.response?.data?.detail || 'Error al eliminar cuello');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Cuellos</h2>
          <p className="text-muted-foreground">
            Tipo de cuello por tipo de prenda (V, Redondo, etc.)
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={4} className="text-center py-8 text-muted-foreground">No hay cuellos registrados</TableCell></TableRow>
              ) : (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingItem ? 'Editar Cuello' : 'Nuevo Cuello'}</DialogTitle>
            <DialogDescription>
              {editingItem ? 'Modifica los datos del cuello' : 'Agrega un nuevo cuello al catálogo'}
            </DialogDescription>
          </DialogHeader>
