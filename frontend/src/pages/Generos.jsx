...
export const Generos = () => {
  ...
  const { sensors, handleDragEnd, isSaving, modifiers } = useSortableTable(items, setItems, 'generos');
  ...
  useEffect(() => { fetchItems(); fetchMarcas(); }, []);

  const handleSubmit = guard(async (e) => {
    e.preventDefault();
    try {
      if (editingItem) {
        await axios.put(`${API}/generos/${editingItem.id}`, formData);
        toast.success('Género actualizado');
      } else {
        await axios.post(`${API}/generos`, formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || 'Error al eliminar género');
    }
  };
  ...
  const toggleMarca = (marcaId) => {
    ...
    setFormData({
      ...formData,
      marca_ids: current.includes(marcaId)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Géneros</h2>
          <p className="text-muted-foreground">
            Clasificación por género (Hombre / Mujer)
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={4} className="text-center py-8 text-muted-foreground">No hay géneros registrados</TableCell></TableRow>
              ) : (
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-1">
                          {(item.marca_ids || []).map(marcaId => {
                            ...
                            return marca ? <Badge key={marcaId} variant="secondary" className="text-xs">{marca.nombre}</Badge> : null;
                          })}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingItem ? 'Editar Género' : 'Nuevo Género'}</DialogTitle>
            <DialogDescription>
              {editingItem ? 'Modifica los datos del género' : 'Agrega un nuevo género al catálogo'}
            </DialogDescription>
          </DialogHeader>
                      ...
                      <div className="flex flex-wrap gap-1 flex-1">
                        {formData.marca_ids?.length === 0 ? (
                          ...
                          formData.marca_ids.map(id => {
                            ...
                            return marca ? <Badge key={id} variant="secondary" className="text-xs">{marca.nombre}</Badge> : null;
                          })
                        )}
                      </div>
