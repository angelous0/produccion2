...
export const Detalles = () => {
  ...
  const { sensors, handleDragEnd, isSaving, modifiers } = useSortableTable(items, setItems, 'detalles');

  const fetchItems = async () => {
    try {
      const res = await axios.get(`${API}/detalles`);
      setItems(res.data);
    } catch {
  ...
  const handleSubmit = guard(async (e) => {
    ...
    try {
      if (editingItem) {
        await axios.put(`${API}/detalles/${editingItem.id}`, formData);
        toast.success('Detalle actualizado');
      } else {
        await axios.post(`${API}/detalles`, formData);
      ...
      toast.error(err.response?.data?.detail || 'Error al guardar detalle');
    }
  });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API}/detalles/${id}`);
      toast.success('Detalle eliminado');
      fetchItems();
    } catch (err) {
      toast.error(err.response?.data?.detail || 'Error al eliminar detalle');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Detalles adicionales de clasificación por tipo de prenda
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={4} className="text-center py-8 text-muted-foreground">No hay detalles registrados</TableCell></TableRow>
              ) : (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingItem ? 'Editar Detalle' : 'Nuevo Detalle'}</DialogTitle>
            <DialogDescription>
              {editingItem ? 'Modifica los datos del detalle' : 'Agrega un nuevo detalle al catálogo'}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Ej: Estampado, Bordado, Liso"
                  required
                />
              </div>
              <div className="space-y-2">
